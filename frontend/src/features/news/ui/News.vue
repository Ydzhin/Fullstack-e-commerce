...
async function getUserDataByPost() {
  await axios.get(`${import.meta.env.VITE_APP_API}/get_user_data_by_post`, {
    params: {
...
async function getUserData() {
  if (token) {
    axios.get(`${import.meta.env.VITE_APP_API}/get_user_data?id=${id}`).then((resp) => {
...
async function getLikesByPost() {
  await axios.get(`${import.meta.env.VITE_APP_API}/get_likes_by_post`, {
    params: {
      post_id: props._id,
    },
  }).then((resp) => {
...
async function like() {
  if (token) {
    if (!wrongLike.value) {
      await axios.post(`${import.meta.env.VITE_APP_API}/create_like`, {
        post_id: props._id,
        user_id: userData.value._id,
      });
    } else {
      await axios.post(`${import.meta.env.VITE_APP_API}/delete_like`, {
        post_id: props._id,
      }).then(() => {
        wrongLike.value = false;
      });
