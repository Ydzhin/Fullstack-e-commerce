...
async function getPosts() {
  await axios.get(`${import.meta.env.VITE_APP_API}/post/get_by_userId/${props.id}`).then((resp) => {
...
async function deletePost(post_id: string) {
  await axios.delete(`${import.meta.env.VITE_APP_API}/post/delete/${post_id}`).then((resp) => {
...
async function createPost(event: Event) {
  ...
  if (text.value.length < 6) {
    ...
    axios.post(`${import.meta.env.VITE_APP_API}/post/create`, {
      userId: props.id,
