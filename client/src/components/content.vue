  ...
  <div class="main">
    <div class="filter">
      ...
      <categoriesList
        ...
        :key="categorie"
        @click="sort_by_category(categorie)"
      >
        {{ categorie }}
      </categoriesList>
    </div>
    <main>
      ...
      <div class="products">
        <card
          ...
          :url="product.image"
          :alt="product.title"
          :title="hide_long_text(product.title)"
          :price="product.price"
          :category="product.category"
          @handle_like="handle_like"
          @dis_like="remove_like"
          @item_clicked="add_item(product)"
...
import { useSort } from "@/store/sort.store.ts";
...
const sort_by_category = (e) => {
  ...
  if (
    JSON.parse(JSON.stringify(store_categories.selected_categories)).length !==
    0
  ) {
    filtered_products.value = JSON.parse(JSON.stringify(products.value)).filter(
      (el) => {
...
onMounted(async () => {
  ...
  if (store_categories.getAllCategories.length === 0) {
    await axios
      ...
      .then((response) => {
        store_categories.add_all_categories(
