    ...
    <div>
      <div class="topmodal">
        ...
        <div class="details">
          ...
          <p style="color: gray">
            À noter qu'aucun article ne sera retourné au delà de 30 jours à
            compter de la date de livraison effective.
          </p>
          <p style="color: gray">
            Conctactez le <a href="">service client</a> pour toute information
            supplémentaire
          </p>
        </div>
      </div>
      ...
      <modalCard
        v-for="item_in_cart in store_cart.getItems"
        :image="item_in_cart.item.image"
        :quantity="item_in_cart.number"
        :computedPrice="item_in_cart.number * item_in_cart.item.price"
      >
        {{ item_in_cart.item.title }}
        <template v-slot:reduceQuantity>
          <button class="handleQuantity" @click="reduceQuantity(item_in_cart)">
            -
          </button>
        </template>
        <template v-slot:addQuantity>
          <button class="handleQuantity" @click="addQuantity(item_in_cart)">
            +
          </button>
        </template>
      ...
      <div class="promo" v-if="store_cart.getItems.length !== 0">
        <span
          ><input type="checkbox" v-model="have_promo" /> I have a promo code </span
        ><br />
        <input
          type="text"
          v-if="have_promo"
      ...
      <button
        class="pay"
        @click="goPayement"
        v-if="store_cart.getItems.length !== 0"
      >
        <svg
          class="iconLock"
          width="24"
...
import { useCart } from "@/store/cart.store.ts";
import { element_index_in_array } from "@/services/utils/utils.ts";
...
const deleteItem = (e) => {
  store_cart.delete_item(
    element_index_in_array(JSON.parse(JSON.stringify(store_cart)).getItems, e)
  );
};
const reduceQuantity = (e) => {
  store_cart.decrease_number(
    element_index_in_array(JSON.parse(JSON.stringify(store_cart)).getItems, e)
  );
};
const addQuantity = (e) => {
  store_cart.increase_number(
    element_index_in_array(JSON.parse(JSON.stringify(store_cart)).getItems, e)
  );
